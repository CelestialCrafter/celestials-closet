...
const idle = (state) => () => {
  ...
  const { idleAnimationFrame: frame } = state;
  switch (state.idleAnimation) {
    case "sleeping":
      if (frame < 8) {
        ...
        if (frame > 192) resetIdleAnimation();
      }

      break;
    case "scratchSelf":
      setSprite(frame, frame);
...
const oneko = () => {
  ...
  init();
  nekos.push(tick(state, idle(state), setSprite(state.element)));
};
